...
- **正文识别**：结合服务或资源、地域邀约与联系动作判断，支持相邻短句、换行中的推广组合。仅提到“约炮”“裸聊”或 NSFW 等话题词，默认保留。
- **依据可查**：正文招揽会展示实际命中的词组和片段，方便核对原因、展开原文。
...
在「设置 → 内置规则」点击「更新规则」，即可获取新的过滤词库和规则说明，查看当前版本与最近检查时间。更新后，已打开的 X 页面和体验页会自动使用新规则。

- 词库可以单独更新，不必为每次规则调整重新安装扩展。
- 下载失败时继续使用原规则；下载成功后，断网也能继续过滤。
- 可以随时「恢复内置规则」，自己的关键词、黑白名单和规则开关不会被覆盖。

规则只在你点击更新时联网下载，不会上传浏览的帖子或个人名单。新增识别方式仍可能需要更新扩展。
